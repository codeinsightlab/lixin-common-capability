...
The Netease IM starter supports Spring Boot 3/4 auto configuration through:

```text
META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports
```

It also keeps `META-INF/spring.factories` for Spring Boot 2.x compatibility. Do not add a long-term business-project `@Import` bridge for `LixinNeteaseImAutoConfiguration`.
